...
from tests.helpers.util import print_lines, wait_for, copy_file_into_container, container_ip
...
INITIAL_VERSION = os.environ.get("INITIAL_VERSION", "4.7.5-1")
UPGRADE_VERSION = os.environ.get("UPGRADE_VERSION", "4.7.6-1")


def get_config(cont, backend, monitors, agent_version, stage):
    config_yaml = yaml.safe_load(CONFIG)
    config_yaml["internalStatusHost"] = container_ip(cont)
...
def run_ansible(base_image, cont, backend, monitors, agent_version, stage):
    with tempfile.NamedTemporaryFile(mode="w+") as fd:
        config_yaml = get_config(cont, backend, monitors, agent_version, stage)
        print(config_yaml)
